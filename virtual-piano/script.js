...
function addActiveClass(target, activeClass) {
  const array =
    activeClass === PIANO_ACTIVE_CLASS
      ? pianoKeys
      : activeClass === BTN_ACTIVE_CLASS
      ? buttons
      : [];
...
// we remove class 'piano-key-active' from mouse up and mousout
function removeActiveKey(key) {
  console.log('removeActiveKey ', key.classList);
...
piano.addEventListener('mousedown', (e) => (isDown = true));
...
piano.addEventListener('mouseout', (e) => {
  if (isDown && e.which === 1) {
    ...
    //   key.addEventListener('transitionend', removeActiveKey)
    // );
    pianoKeys.forEach((key) => removeActiveKey(key));
  }
});
...
function callSoundFromKeyboard(e) {
  ...
  if (letter !== undefined && letter.length === 1) {
    const noteTarget = [...pianoKeys].filter(
      (k) => k.dataset.letter === letter
    )[0];
